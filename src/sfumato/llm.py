...
async def _invoke_sdk_completion(
    ...
    # Ensure API key env var is set. Priority: env var > config api_key > aliases
    provider_env_map = {
        "openrouter": "OPENROUTER_API_KEY",
        "google": "GOOGLE_API_KEY",
        "openai": "OPENAI_API_KEY",
    }
    env_key = provider_env_map.get(ai_config.sdk_provider, "")
    if env_key and not os.environ.get(env_key):
        # Try config api_key first
        if hasattr(ai_config, "api_key") and ai_config.api_key:
            os.environ[env_key] = ai_config.api_key
        elif ai_config.sdk_provider == "openrouter":
            # Try common aliases
